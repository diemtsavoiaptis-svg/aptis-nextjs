...
import part2Rows from "./data.json";
...
function normalizeRow(row, index) {
  const base = emptyRow(index);
  const answerBank = row.answerBank || "";
  const voiceParagraph = row.voiceParagraph || row.paragraph || row.transcript || "";
  ...
  return {
    ...
    stt: String(row.stt || row.question || row.order || index + 1),
    question: String(row.stt || row.question || row.order || index + 1),
    order: Number.parseInt(String(row.stt || row.question || row.order || index + 1), 10) || index + 1,
    audio: row.audioLink || row.audio || "",
    audioLink: row.audioLink || row.audio || "",
    answerBank,
...
const columns = [
  ...
  { key: "stt", label: "STT" },
  { key: "audioLink", label: "Audio" },
  { key: "topic", label: "Chủ đề" },
  { key: "answerBank", label: "Kho đáp án" },
  { key: "person1", label: "Người 1", type: "answerSelect" },
  { key: "person2", label: "Người 2", type: "answerSelect" },
  { key: "person3", label: "Người 3", type: "answerSelect" },
  { key: "person4", label: "Người 4", type: "answerSelect" },
  { key: "voiceParagraph", label: "Paragraph" },
];

export default function AdminListeningPart2Page() {
  const [rows, setRows] = useState(() =>
    Array.isArray(part2Rows) && part2Rows.length
      ? part2Rows.map(normalizeRow)
      : [emptyRow(0)]
  );
  ...
  const answerRows = rows.filter((row) => String(row.answerBank || "").trim()).length;
  ...
  async function saveAll() {
    try {
      ...
      const res = await fetch("/api/admin/part2/save", {
        method: "POST",
        headers: {
      ...
      alert(`Đã lưu Part 2 thành công. Hiện khách: ${data.guestCount}/${data.count} bài.`);
    } catch (error) {
      console.error(error);
      alert("Không lưu được Part 2. Vui lòng xem lỗi trong terminal.");
    } finally {
      setSaving(false);
    }
  }
      ...
      <section className="shell">
        <header className="hero">
          ...
          <div className="heroText">
            ...
            <span>Quản lý STT, audio, chủ đề, kho đáp án, người 1–4 và paragraph.</span>
          </div>
        ...
        <section className="statsGrid">
          ...
          <div className="statCard">
            <span>CÓ KHO ĐÁP ÁN</span>
            <strong>{answerRows}</strong>
          </div>
        ...
        <section className="guestCountPanel">
          ...
          <div className="viewButtons">
            <Link href="/listening/part-2?mode=student">Giao diện học viên</Link>
            <Link href="/listening/part-2">Giao diện khách</Link>
          </div>
        </section>
